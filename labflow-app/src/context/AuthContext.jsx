...
export const ROLES = {
  ...
  nurse:        { id: 'NS-4521', name: 'Patricia Wilson',    title: 'Registered Nurse',    initials: 'PW', color: '#005454', home: '/nurse/triage' },
  patient:      { id: 'PT-0001', name: 'Robert Johnson',    title: 'Patient',             initials: 'RJ', color: '#005454', home: '/' },
}
...
export function AuthProvider({ children }) {
  ...
  const login = async (id, password) => {
    try {
      ...
      if (errorMessage.includes('User ID not found') || errorMessage.includes('404')) {
        toast.error('User ID not found in the system')
      } else if (errorMessage.includes('Incorrect password') || errorMessage.includes('401')) {
        toast.error('Incorrect password')
      } else if (errorMessage.includes('Invalid ID format') || errorMessage.includes('400')) {
        toast.error('Invalid ID format. Please use formats like DR-1024, AD-0001, etc.')
      } else {
