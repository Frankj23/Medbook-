...
export async function initDB() {
  ...
  try {
    ...
    console.warn(
      "Failed to initialize DB cache from API, continuing with local data",
      error,
    );
  }
  ...
  if (isBrowser()) {
    window.addEventListener("online", () => {
      syncPendingOperations().catch((err) =>
        console.warn("Sync after reconnect failed", err),
      );
    });
  }
}
...
export function getStats() {
  ...
  return {
    ...
    inProgress: patients.filter((p) =>
      ["consulting", "lab_pending", "sample_collected"].includes(p.status),
    ).length,
    completedToday: patients.filter((p) =>
      ["prescribed", "discharged"].includes(p.status),
    ).length,
    pendingLab: labReqs.filter((r) =>
      ["pending", "sample_collected"].includes(r.status),
    ).length,
...
export async function fetchLabOrders(query) {
  const response = await request(`/lab/orders${buildQueryString(query)}`);
  const items = extractData("lab_orders", response);
  cache.lab_orders = items;
  saveCache();
  return items;
}
...
export async function fetchConsultations(query) {
  const response = await request(`/consultations${buildQueryString(query)}`);
  const items = extractData("consultations", response);
  cache.consultations = items;
  saveCache();
  return items;
}
...
export async function fetchPatients() {
  const response = await request(`/patients${buildQueryString()}`);
  const items = extractData("patients", response);
  cache.patients = items;
  saveCache();
  return items;
}
...
export async function fetchPrescriptions(query) {
  const response = await request(`/prescriptions${buildQueryString(query)}`);
  const items = extractData("prescriptions", response);
  cache.prescriptions = items;
  saveCache();
  return items;
}
