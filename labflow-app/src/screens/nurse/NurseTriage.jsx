...
export default function NurseTriage() {
  ...
  const handleSearch = () => {
    ...
    const found = patients.find(
      p => p.id.toUpperCase() === q || p.name.toLowerCase().includes(search.toLowerCase())
    )
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f6fafa', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="lf-card" style={{ padding: '52px 48px', maxWidth: '460px', width: '100%', textAlign: 'center' }}>
          ...
          {bmi && (
            <div style={{ background: '#f0f7f7', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', color: '#8fa8a8', fontWeight: '700' }}>BMI</span>
              <span style={{ fontSize: '18px', fontWeight: '800', color: '#005454' }}>{bmi} <span style={{ fontSize: '11px', color: '#8fa8a8' }}>kg/m²</span></span>
            </div>
          )}
          <div style={{ display: 'flex', gap: '12px' }}>
            <button className="btn-ghost" style={{ flex: 1, justifyContent: 'center' }} onClick={() => {
              setPatient(null); setSearch('');
              setVitals({ bp: '', temp: '', pulse: '', weight: '', height: '', spo2: '' });
              setCategory('green'); setSaved(null)
            }}>
              Next Patient
            </button>
    ...
    <div style={{ minHeight: '100vh', background: '#f6fafa', fontFamily: 'Inter, sans-serif' }}>
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', height: '60px', background: '#fff', borderBottom: '1px solid #e8f2f2', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button onClick={logout} style={{ padding: '6px 12px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '11px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
            Logout
          </button>
        </div>
      </nav>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '20px' }}>
            ...
            <div className="lf-card" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
                {[
                  { key: 'bp',     label: 'Blood Pressure', unit: 'mmHg', placeholder: '120/80',  type: 'text' },
                  { key: 'temp',   label: 'Temperature',    unit: '°C',   placeholder: '36.5',    type: 'number' },
                  { key: 'pulse',  label: 'Pulse Rate',     unit: 'bpm',  placeholder: '72',      type: 'number' },
                  { key: 'weight', label: 'Weight',         unit: 'kg',   placeholder: '70',      type: 'number' },
                  { key: 'height', label: 'Height',         unit: 'cm',   placeholder: '170',     type: 'number' },
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ fontSize: '12px', fontWeight: '600', color: '#5a7272', display: 'block', marginBottom: '6px' }}>
                    BMI <span style={{ fontWeight: '400', color: '#aac0c0' }}>(auto-calculated: weight ÷ height²)</span>
                  </label>
                  <div style={{ position: 'relative' }}>
                    <input
                      className="lf-input"
                    ...
                    />
                    <span style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '11px', fontWeight: '700', color: '#8fa8a8' }}>kg/m²</span>
                  </div>
                  {bmi && (
                    <p style={{ fontSize: '11px', color: '#5a7272', margin: '4px 0 0' }}>
                      {parseFloat(bmi) < 18.5
                        ? '⚠ Underweight'
                        : parseFloat(bmi) < 25
                        ? '✓ Normal weight'
              ...
              <div style={{ marginBottom: '24px' }}>
                <label style={{ fontSize: '12px', fontWeight: '700', color: '#5a7272', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  {CATEGORIES.map(c => (
                    ...
                    >
                      {/* Colored dot */}
                      <span style={{ width: '14px', height: '14px', borderRadius: '50%', background: c.dot, display: 'block', boxShadow: category === c.key ? `0 0 8px ${c.dot}88` : 'none', transition: 'box-shadow 0.15s' }} />
                      {/* Level badge */}
                      <span style={{
                        fontSize: '10px',
                        fontWeight: '800',
                      ...
                      <span style={{ fontSize: '11px', fontWeight: '700' }}>{c.label.split('—')[1]?.trim()}</span>
                      <span style={{ fontSize: '10px', fontWeight: '500', opacity: 0.7 }}>{c.sublabel}</span>
                    </button>
                  ))}
                </div>
              </div>
