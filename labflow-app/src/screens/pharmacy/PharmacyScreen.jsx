...
export default function PharmacyScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [prescriptionsData, patients] = await Promise.all([
          fetchPrescriptions(),
          fetchPatients(),
        ]);
        const patientMap = new Map(patients.map((p) => [p.id, p]));
        const loaded = prescriptionsData.map((item) => ({
          ...item,
          patient:
            item.patient ||
            patientMap.get(item.patientId) ||
            getById("patients", item.patientId),
          medications: item.medications || [],
        }));
        setPrescriptions(loaded);
        if (!selectedPrescription && loaded.length > 0) {
          setSelectedPrescription(loaded[0]);
          setUpdatedMedications(
            loaded[0].medications.reduce((acc, med, i) => {
              acc[i] = { ...med };
              return acc;
            }, {}),
          );
  ...
  const handleSelectPrescription = (prescription) => {
    ...
    setUpdatedMedications(
      prescription.medications.reduce((acc, med, i) => {
        acc[i] = { ...med };
        return acc;
      }, {}),
    );
  ...
  const persistPrescription = async (updatedPrescription) => {
    try {
      await updatePrescription(updatedPrescription.id, {
        medications: Object.values(updatedMedications),
        status: updatedPrescription.status,
        pharmacyNote: updatedPrescription.pharmacyNote,
        updatedAt: new Date().toISOString(),
      });
    } catch (error) {
      console.warn("Failed to persist prescription update", error);
    }
  };

  const handlePrescriptionAction = async (status) => {
    ...
    await persistPrescription(updatedPrescription);
    if (status === "Completed") {
      notifyDoctor(updatedPrescription.id, "Completed");
      alert("Prescription confirmed and doctor notified!");
    } else if (status === "Rejected") {
      alert("Prescription rejected and recorded in database.");
    } else if (status === "Partial") {
      alert("Prescription partially fulfilled and saved.");
    }
  };

  const handleConfirm = async () => {
    await handlePrescriptionAction("Completed");
  };
  ...
  const notifyDoctor = (prescriptionId, status) => {
    console.log(
      `Notifying doctor about prescription ${prescriptionId} with status ${status}`,
    );
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            ...
            <span
              style={{ fontSize: "17px", fontWeight: "800", color: "#005454" }}
            >
              LabFlow
            </span>
          </div>
          ...
          {selectedPrescription ? (
            <>
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <select
                          value={med.status}
                          onChange={(e) =>
                            updateMedication(i, "status", e.target.value)
                          }
                          style={{
                            width: "100%",
                            padding: "8px 10px",
                    ...
                    <div style={{ marginBottom: "12px" }}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateMedication(i, "substitute", e.target.value)
                        }
                        style={{
                          width: "100%",
                          padding: "8px 10px",
                    ...
                    <div>
                      ...
                      <textarea
                        value={med.note}
                        onChange={(e) =>
                          updateMedication(i, "note", e.target.value)
                        }
                        rows={2}
                        style={{
                          width: "100%",
              ...
              >
                ...
                >
                  <button
                    onClick={() => handlePrescriptionAction("Completed")}
                    style={{
                      padding: "10px 16px",
                      background: "#16a34a",
                      color: "#fff",
                  ...
                  <button
                    onClick={() => handlePrescriptionAction("Partial")}
                    style={{
                      padding: "10px 16px",
                      background: "#d97706",
                      color: "#fff",
                  ...
                  <button
                    onClick={() => handlePrescriptionAction("Rejected")}
                    style={{
                      padding: "10px 16px",
                      background: "#dc2626",
                      color: "#fff",
            ...
            >
              <p style={{ fontSize: "16px", color: "#8fa8a8" }}>
